...
class NotificationMailer < ActionMailer::Base
  ...
  def feature_updated(feature)
    ...
    mail(:to => recipients,
         :subject => t("txt.notification_mailer.feature_updated_subject", :project_name => feature.project.name, :project_feature_id => feature.project_feature_id, :title => feature.title, :updater_name => feature.updated_by.name)) do |format|
          format.html
          format.text
         end
    end
